...
const Khata = () => {
  ...
  useEffect(() => {
    ...
    if (cachedRaw) {
      try {
        ...
        // Robust check to handle both array and object structures
        setCustomers(Array.isArray(parsed) ? parsed : (parsed.customers || []));
      } catch (e) { console.error("Cache corrupted"); }
    }
    fetchCustomers(); // Refresh in background
  ...
  // Helper to maintain the new Master JSON structure (customers + history)
  ...
  // 🚀 2. SYNC BACKGROUND DATA TO MASTER JSON
  const fetchCustomers = async () => {
    try {
      const { data, error } = await supabase.from('customers').select('*').order('name');
      if (data) {
        setCustomers(data);
        updateMasterCache(data);
      }
    } catch (err) {
  ...
  const handleRefresh = async (customerId) => {
    ...
    if (updatedCust) {
        const newList = customers.map(c => c.id === customerId ? updatedCust : c);
  ...
  const handleQuickSave = async (e) => {
    ...
    try {
      ...
      // 🚀 3. OPTIMISTIC LOCAL JSON UPDATE
      const updatedList = customers.map(c => c.id === customer.id ? {...c, balance: newBalance} : c);
      ...
      setIsEntryOpen(false); fetchCustomers(); showToast("Success! ✅");
        ...
        fetchCustomers(); // Rollback local cache on error
    }
    setIsSaving(false);
  };
    ...
    <div style={containerStyle} className="khata-container">
      <style>{`
        ...
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        @media (min-width: 768px) {
          .khata-container { max-width: 700px !important; }
          .header-closed { padding: 18px 25px !important; }
          .customer-row { padding: 14px 22px !important; }
          .dialog-card { max-width: 400px !important; padding: 30px !important; }
        }
      `}</style>
      ...
      <div style={unifiedListBox}>
        {customers.filter(c => c.name.toLowerCase().includes(search.toLowerCase())).map(c => (
          <div key={c.id} style={customerRow} className="customer-row">
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flex: 1 }} onClick={() => openHistory(c)}>
              <div style={avatarCircle}>{c.name[0].toUpperCase()}</div>
              <strong style={{ fontSize: '1rem', textTransform: 'capitalize', color: '#eee', fontWeight: '600' }}>{c.name}</strong>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
              <span style={{ color: c.balance > 0 ? '#ff4d4d' : '#4caf50', fontWeight: '900', fontSize: '1rem' }} onClick={() => openHistory(c)}>₹{c.balance}</span>
              <div style={{ display: 'flex', gap: '12px', borderLeft: '1px solid #333', paddingLeft: '12px' }}>
                <Edit3 size={17} color="#444" onClick={() => openRename(c)} style={{ cursor: 'pointer' }} />
                <Trash2 size={17} color="#555" onClick={() => openDelete(c)} style={{ cursor: 'pointer' }} />
              </div>
            </div>
          </div>
        ))}
          ...
          <div style={dialogCard} className="dialog-card">
            ...
            <div style={{textAlign: 'center', width: '100%'}}>
              ...
              <div style={dialogSubTitle}>
                {dialog.type === 'rename' ? (
                  <p>Update <span style={{color: '#fff'}}>"{dialog.data.name.toUpperCase()}"</span> to <br/> <span style={{color: '#4caf50'}}>"{dialog.value.toUpperCase()}"</span></p>
                ) : (
                  dialog.step === 0 ? (
                    <div>
                      <p>Wiping <span style={{color: '#ff4d4d', fontWeight: '900'}}>"{dialog.data.name.toUpperCase()}"</span></p>
                      <p style={{fontSize: '0.85rem', marginTop: '10px', opacity: 0.8}}>History & ₹{dialog.data.balance} will be lost permanently.</p>
                    </div>
                  ) : (
                    <p>Enter security PIN for <br/> <span style={{color: '#fff'}}>{dialog.data.name.toUpperCase()}</span></p>
                  )
                )}
              </div>
            </div>
            {(dialog.type === 'rename' || dialog.step === 1) && (
                <input placeholder={dialog.type === 'rename' ? "New Name..." : "Enter PIN"} value={dialog.type === 'rename' ? dialog.value : dialog.pin} onChange={e => dialog.type === 'rename' ? setDialog({...dialog, value: e.target.value}) : setDialog({...dialog, pin: e.target.value})} style={{...dialogInput, letterSpacing: dialog.type === 'delete' ? '10px' : 'normal'}} autoFocus type={dialog.type === 'rename' ? "text" : "password"} />
            )}
            {dialog.error && <p style={{color:'#ff4d4d', fontSize:'0.75rem', fontWeight:'900', marginTop:'10px'}}>{dialog.error}</p>}
            <div style={dialogActions}>
              ...
              <button onClick={handleDialogSubmit} style={dialogBtn(dialog.type === 'delete' ? '#ff4d4d' : '#4caf50', '#000')}>{dialog.type === 'delete' && dialog.step === 0 ? 'DELETE NOW' : 'CONFIRM'}</button>
            </div>
          </div>
        </div>
      )}
...
const customerRow = { display: 'flex', justifyContent: 'space-between', padding: '14px 18px', alignItems: 'center', borderBottom: '1px solid #222' };
const avatarCircle = { minWidth: '36px', height: '36px', borderRadius: '50%', background: '#000', display: 'flex', justifyContent: 'center', alignItems: 'center', fontWeight: 'bold', color: '#4caf50', fontSize: '0.9rem' };
...
const dialogOverlay = { position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.95)', backdropFilter: 'blur(12px)', zIndex: 10000, display: 'flex', justifyContent: 'center', alignItems: 'flex-start', padding: '60px 20px', overflowY: 'auto' };
const dialogCard = { width: '100%', maxWidth: '340px', background: '#1a1a1a', border: '1.5px solid #333', borderRadius: '35px', padding: '35px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', margin: 'auto 0' };
